  ...
  group('SearchRidePage', () {
    ...
    group('Search', () {
      ...
      testWidgets('Normal input', (WidgetTester tester) async {
        ...
        // `rides: []` is needed here and everywhere else because otherwise, the user might have a ride with the drive.
        // This would cause this ride's data (that has been randomly generated by the factory) to be used instead of the drive's that is provided here.
        // And in general, it's not important whether the drives of search results have rides or not.
        final List<Map<String, dynamic>> drives = [
          DriveFactory().generateFake(startDateTime: startDateTime, rides: []).toJsonForApi(),
          DriveFactory()
              .generateFake(startDateTime: startDateTime.add(const Duration(hours: 1)), rides: []).toJsonForApi(),
          DriveFactory()
              .generateFake(startDateTime: startDateTime.add(const Duration(days: 1)), rides: []).toJsonForApi(),
        ];
    ...
    group('Filter', () {
      ...
      group('Sort', () {
        ...
        testWidgets('Relevance (whole day)', (WidgetTester tester) async {
          final DateTime now = DateTime.now();
          final DateTime startDateTime = DateTime(now.year, now.month, now.day);
          ...
          final List<Drive> drives = [
            ...
            DriveFactory().generateFake(
                ...
                destinationPosition: Position(latDiffForKm(10), 0),
                rides: []),
            //Time proximity on the same day is ignored in wholeDay, so this is worse because it's longer
            DriveFactory().generateFake(
                id: 2,
                startDateTime: startDateTime.add(const Duration(minutes: 50)),
                destinationDateTime: startDateTime.add(const Duration(minutes: 111)),
                ...
                destinationPosition: Position(latDiffForKm(10), 0),
                rides: []),
            //If the day is different, the penalty is the difference in minutes, so this will be much worse
            DriveFactory().generateFake(
                id: 3,
                startDateTime: startDateTime.add(const Duration(days: 1)),
                destinationDateTime: startDateTime.add(const Duration(days: 1, hours: 1)),
                startPosition: Position(0, 0),
                destinationPosition: Position(latDiffForKm(10), 0),
                rides: []),
          ];
        ...
        testWidgets('Price', (WidgetTester tester) async {
          final List<Drive> drives = [
            DriveFactory()
                .generateFake(startPosition: Position(0, 0), destinationPosition: Position(latDiffForKm(10), 0)),
            DriveFactory()
                .generateFake(startPosition: Position(0, 0), destinationPosition: Position(latDiffForKm(5), 0)),
            DriveFactory()
                .generateFake(startPosition: Position(0, 0), destinationPosition: Position(latDiffForKm(8), 0)),
          ];
          final List<Map<String, dynamic>> driveJsons = drives.map((Drive drive) => drive.toJsonForApi()).toList();
